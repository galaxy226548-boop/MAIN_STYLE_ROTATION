"""Interactive builder for grouped binary COMB signal matrix.

Usage:
    python F_grouping/scripts/build_signal_group_binary_interactive.py
...
Output for factor group W004:
    F_grouping/output_COMB/W004_signal_binary_mounted_normalized_factors.parquet
    F_grouping/output_COMB/W004_signal_binary_signal_ls.parquet
    F_grouping/output_COMB/W004_signal_binary_mounted_normalized_factors.xlsx
    F_grouping/output_COMB/W004_signal_binary_signal_ls.xlsx
    F_grouping/output_COMB/W004_signal_binary_factor_grouping_record.json

Signal logic:
    1. Optionally forward-fill each source signal_ls column before aggregation.
    2. Group source signal_ls columns by first letter, then equal-weight average within each group.
    3. Equal-weight average all first-level groups into a continuous signal_group column.
    4. Binarize: mean > 0 → 1.0, mean < 0 → -1.0, mean == 0 → carry forward
       the previous period's binary signal. NaN source rows remain NaN.

mounted_normalized_factors columns: [source signals..., first-level groups..., {signal_group}, {binary_group}]
signal_ls columns:                  [source signals..., first-level groups..., {signal_group}, {binary_group}]
"""
...
def normalize_factor_group(raw_factor_group: str) -> tuple[str, str, str]:
    ...
    elif factor_group.endswith(SIGNAL_SUFFIX):
    ...
    signal_group = f"{base_group}{SIGNAL_SUFFIX}"
    binary_group = f"{base_group}{BINARY_SUFFIX}"
    return base_group, signal_group, binary_group
...
# ========== 4. Build grouped binary signal matrix ==========
...
def build_factor_matrix(
    ...
    """Compute first-letter grouped means, final continuous signal, and binary signal.

    Returns factor_df with columns:
    [*source_cols, *first_level_group_cols, {signal_group}, {binary_group}].
    """
    warnings: list[str] = []
    ...
    first_level_df = pd.DataFrame(index=source_numeric.index)
    for letter, columns in groups.items():
        ...
        first_level_df[factor_col] = source_numeric[columns].mean(axis=1, skipna=True)
    ...
    continuous_signal = first_level_df.mean(axis=1, skipna=True).rename(signal_group)
    binary_signal = binarize_series(continuous_signal, binary_group, warnings).rename(binary_group)

    factor_df = pd.concat(
        [source_numeric, first_level_df, continuous_signal, binary_signal],
        axis=1,
    ).astype("float64")
...
def build_signal_ls_matrix(factor_df: pd.DataFrame) -> pd.DataFrame:
    """Keep signal_ls identical to mounted_normalized_factors for this grouped binary output."""
    return factor_df.copy().astype("float64")
...
def build_record(
    ...
    binarization_logic = (
        f"{binary_group} = 1.0 if {signal_group} > 0; "
        f"-1.0 if {signal_group} < 0; "
        f"carry forward previous period binary signal if {signal_group} == 0; "
    ...
    for letter, columns in groups.items():
        ...
        first_level_factors[factor_col] = {
            ...
            "signal_ls_logic": "signal_ls equals the factor value exactly; no additional transformation is applied.",
    ...
    signal_non_null = int(factor_df[signal_group].notna().sum()) if signal_group in factor_df else 0
    ...
    return {
        ...
        "grouping_rule": (
            "Selected source signals are optionally forward-filled, grouped by the first character "
            "of each factor name, averaged into a continuous signal group, then binarized."
        ),
        ...
        "continuous_signal": {
            ...
            "factor_value_logic": continuous_logic,
            "signal_ls_logic": "signal_ls equals the factor value exactly; no additional transformation is applied.",
            "non_null_count": signal_non_null,
        },
        "binary_signal": {
            "name": binary_group,
            "source_continuous_signal": signal_group,
...
def main() -> None:
    ...
    base_group, signal_group, binary_group = normalize_factor_group(input_factor_group)
